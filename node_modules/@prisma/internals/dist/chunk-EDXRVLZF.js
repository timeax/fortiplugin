...
var chunk_EDXRVLZF_exports = {};
__export(chunk_EDXRVLZF_exports, {
...
module.exports = __toCommonJS(chunk_EDXRVLZF_exports);
...
var import_chunk_RGSVHAVU = require("./chunk-RGSVHAVU.js");
var import_chunk_J3UTRQA4 = require("./chunk-J3UTRQA4.js");
var import_chunk_LAYR2CL2 = require("./chunk-LAYR2CL2.js");
var import_chunk_K6UIU25V = require("./chunk-K6UIU25V.js");
var import_chunk_EYVG6XQS = require("./chunk-EYVG6XQS.js");
var import_chunk_HLGYMMNR = require("./chunk-HLGYMMNR.js");
...
async function formatSchema({ schemas }, inputFormattingOptions) {
  if (process.env.FORCE_PANIC_PRISMA_SCHEMA) {
    handleFormatPanic(() => {
      import_chunk_HLGYMMNR.prismaSchemaWasm.debug_panic();
    });
  }
  ...
  const { formattedMultipleSchemas, lintDiagnostics } = handleFormatPanic(() => {
    ...
    const lintDiagnostics2 = (0, import_chunk_LAYR2CL2.lintSchema)({ schemas: formattedMultipleSchemas2 });
    return { formattedMultipleSchemas: formattedMultipleSchemas2, lintDiagnostics: lintDiagnostics2 };
  });
  const lintWarnings = (0, import_chunk_LAYR2CL2.getLintWarningsAsText)(lintDiagnostics);
...
function formatWasm(schema, documentFormattingParams) {
  const formattedSchema = import_chunk_HLGYMMNR.prismaSchemaWasm.format(schema, JSON.stringify(documentFormattingParams));
...
async function processSchemaResult({
  ...
  const configFromPsl = await (0, import_chunk_RGSVHAVU.getConfig)({ datamodel: schemaResult.schemas, ignoreEnvVarErrors });
...
async function getGenerators(options) {
  ...
  const dmmf = await (0, import_chunk_J3UTRQA4.getDMMF)({
...
You need to define \`output\` in the generator block in the schema file.`
        ...
        const datamodel = (0, import_chunk_K6UIU25V.mergeSchemas)({ schemas: schemaContext.schemaFiles });
    ...
    for (const generator of generators) {
      if (generator.manifest && generator.manifest.requiresEngines) {
        ...
        if (engineVersion !== version2 && generator.options && generator.manifest.requiresEngines.includes(queryEngineType) && generatorBinaryPaths[queryEngineType] && generatorBinaryPaths[queryEngineType]?.[binaryTarget]) {
          const customDmmf = await (0, import_chunk_J3UTRQA4.getDMMF)({
            datamodel: schemaContext.schemaFiles,
            previewFeatures
          });
...
function checkUnsupportedDataProxy({
  ...
  const url = (0, import_chunk_RGSVHAVU.resolveUrl)((0, import_chunk_RGSVHAVU.getEffectiveUrl)(schemaContext.primaryDatasource));
