...
export function buildModelContent(model) {
    ...
    model.properties.forEach(p => {
        if (p.cast)
            castsLines.push(`'${p.name}' => '${p.cast}'`);
        else if (p.enumRef)
            castsLines.push(`'${p.name}' => ${p.enumRef}::class`);
        else if (p.phpType)
            castsLines.push(`'${p.name}' => ${p.phpType}`);
    });
    if (castsLines.length) {
        out.push("protected $casts = [" +
            castsLines.join(", ") +
            "];", "");
    }
    /* ---- Touches & Appends -------------------------------------------- */
    ...
    if (Object.keys(model.interfaces).length) {
        const lines = Object.entries(model.interfaces).map(([k, v]) => `'${k}' => [${v.import ? ` 'import' => '${v.import}',` : ""} 'type' => '${v.type}' ]`);
        out.push("public array $interfaces = [" +
            lines.join(", ") +
            "];", "");
    }
    /* ---- Factory helper ----------------------------------------------- */
    if (model.factory) {
        out.push(`protected static string $factory = ${model.factory}::class;`, "");
    }
    /* ---- boot() for observer ------------------------------------------ */
    if (model.observer) {
        out.push("protected static function boot()\n" +
            "{\n" +
            "    parent::boot();\n" +
  ...
  * -----------------------------------------------------------------*/
    ...
    function buildAppendAccessors(appends) {
        if (!appends?.length)
            return [];
        return appends.map(attr => {
            const studly = accessorName(attr);
            return (`public function get${studly}Attribute()
{
    // TODO: compute '${attr}'
