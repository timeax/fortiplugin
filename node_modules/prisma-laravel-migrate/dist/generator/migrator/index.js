...
import { addToConfig } from "../../utils/utils.js";
...
export async function generateLaravelSchema(options) {
    ...
    const loadGroups = async (p) => {
        ...
        if (!Array.isArray(mod)) {
            throw new Error(`Custom groups module must export an array, got ${typeof mod}`);
        }
        return mod;
    };
    if (raw["groups"])
        groups = await loadGroups(raw["groups"]);
    const pick = (key, fallback) => shared.migrate?.[key] ??
        shared[key] ??
        raw[key] ??
        fallback;
    /* -----------------------------------------------------------
    ...
    if (cfg.rules) {
        ...
        if (!Array.isArray(customRules) ||
            !customRules.every(r => r &&
                typeof r.test === 'function' &&
                typeof r.render === 'function')) {
            throw new Error('Custom rules must export an array of objects each having a `test` and `render` method.');
        }
    ...
    active.forEach((mig, idx) => {
        ...
        if (!cfg.noEmit) {
            writeWithMerge(filePath, content, 'migrator', cfg.overwriteExisting ?? false, existingPath // <-- pass old path here
            );
        }
    });
    return migrations;
