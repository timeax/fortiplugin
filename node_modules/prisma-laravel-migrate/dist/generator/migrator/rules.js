...
const timestampsTzRule = {
    test: (def, allDefs) => {
        if (def.name !== "created_at")
            return false;
        const tzTypes = [
        ...
        return (tzTypes.includes(def.migrationType) &&
            allDefs.some(d => d.name === "updated_at" && tzTypes.includes(d.migrationType)));
    },
    render: (def, allDefs) => {
...
const timestampsRule = {
    test: (def, allDefs) => {
        if (def.name !== "created_at")
            return false;
        const plainTypes = [MigrationTypes.dateTime, MigrationTypes.timestamp];
        return (plainTypes.includes(def.migrationType) &&
            allDefs.some(d => d.name === "updated_at" && plainTypes.includes(d.migrationType)));
    },
    render: (def, allDefs) => {
        // ignore the `updated_at` counterpart
        const updated = allDefs.find(d => d.name === "updated_at");
...
const softDeletesTzRule = {
    test: def => def.name === "deleted_at" &&
        [MigrationTypes.dateTimeTz, MigrationTypes.timestampsTz, MigrationTypes.timestamp].includes(def.migrationType),
    render: def => {
        return { column: def.name, snippet: ["$table->softDeletesTz();"] };
    },
};
/** plain soft deletes → softDeletes() */
const softDeletesRule = {
    test: (def, allDefs) => {
        if (def.name !== "deleted_at")
            return false;
        const plainTypes = [MigrationTypes.dateTime, MigrationTypes.timestamp];
        return (plainTypes.includes(def.migrationType) &&
            !allDefs.some(d => d.name === "deleted_at" &&
                [MigrationTypes.dateTimeTz, MigrationTypes.timestampsTz, MigrationTypes.timestamp].includes(d.migrationType)));
    },
    render: def => {
...
const foreignIdRule = {
    test: (def, allDefs) => {
        if (def.local)
            return false; // NEW: skip if silenced at column level
        return intTypes.includes(def.migrationType) &&
            def.name.endsWith("_id") &&
            allDefs.some(item => item.relationFromFields?.length === 1 &&
    ...
    render: (def, allDefs, _d, defaultMaps) => {
        ...
        const ref = allDefs.find(item => item.relationFromFields?.length === 1 &&
            item.relationFromFields.includes(def.name));
        ...
        if (def.hasDefaultValue)
            snippet += formatDefault(def, defaultMaps);
        if (def.comment)
            snippet += `->comment(${JSON.stringify(def.comment)})`;
        ...
        if (ref.relationship.onDelete)
            snippet += `->onDelete('${ref.relationship.onDelete}')`;
        if (ref.relationship.onUpdate)
            snippet += `->onUpdate('${ref.relationship.onUpdate}')`;
        snippet += ';';
...
function runNormal(def, defaultMaps, snippet) {
    ...
    if (def.nullable)
        line += "->nullable()";
    if (def.hasDefaultValue)
        line += formatDefault(def, defaultMaps);
    if (def.comment)
        line += `->comment(${JSON.stringify(def.comment)})`;
    line += ";";
    snippet.push(line);
}
/**
 ...
 */
export function defaultBuild(def, defaultMaps) {
    def.ignore && (def.ignore = true); // leave ignore as set
    let snippet = [];
    if (!def.ignore) { // always render relations
        if (def.local || def.relationship?.local)
            runNormal(def, defaultMaps, snippet);
...
const morphsMergeRule = {
    ...
    render: (def, allDefs) => {
        ...
        const method = def.migrationType === MigrationTypes.uuid
