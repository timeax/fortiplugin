...
const pivotOtherEndpointFor = (thisModelName, candidate) => {
    ...
    if (rels.length !== 2)
        return undefined;
    if (!rels.every((r) => (r.relationFromFields?.length ?? 0) > 0))
        return undefined;
...
export function extractListRelationKeys(dmmf, model, field) {
    if (!field.isList)
        return null;
    const related = getModel(dmmf, field.type);
    const thisTable = dbNameOf(model);
    const relatedTable = dbNameOf(related);
    const counterpart = related.fields.find((r) => r.kind === "object" &&
...
export function buildRelationsForModel(dmmf, model) {
    const defs = [];
    // object relations (belongsTo / hasOne / hasMany / belongsToMany)
    for (const f of model.fields) {
        if (f.kind !== "object" || !f.relationName)
            continue;
        if (isForModel(parseLocalDirective(f.documentation ?? "")))
            continue;
        if (f.isList) {
