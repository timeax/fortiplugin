...
export async function generateLaravelModels(options) {
    ...
    if (raw["groups"]) {
        ...
        if (!Array.isArray(imported)) {
            throw new Error(`Custom groups module must export an array, but got ${typeof imported}`);
        }
        groups = imported;
    }
    /* helper to prefer shared → per-gen → block */
    const pick = (key, fallback) => shared.modeler?.[key] ??
        shared[key] ??
        raw[key] ??
        fallback;
    /* -------- merged config -------- */
    ...
    for (const enumDef of enums) {
        ...
        !cfg.noEmit &&
            writeWithMerge(enumFile, enumPhp, 'model', cfg.overwriteExisting ?? false);
    }
    // 5) Write model files
    for (const model of models) {
        ...
        //----
        if (Array.isArray(model.imports))
        ...
        !cfg.noEmit &&
            writeWithMerge(modelFile, modelPhp, 'model', cfg.overwriteExisting ?? false);
    }
    return { models, enums };
}
