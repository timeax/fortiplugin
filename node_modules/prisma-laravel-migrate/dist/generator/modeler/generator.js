    ...
    generateAll() {
        ...
        const models = this.dmmf.datamodel.models.map(model => {
            ...
            const properties = model.fields.map(field => {
                ...
                if (phpType === 'mixed' || field.relationFromFields?.length || field.relationName)
                    phpType = '';
                if (phpType.endsWith('::class')) {
                    if (!propImps.includes(phpType))
                        propImps.push(phpType.slice(0, -7));
            ...
            const guarded = guardedSet.size || properties.some(p => p.guarded)
                ? [...guardedSet, ...properties.filter(p => p.guarded).map(p => p.name)]
            ...
            for (const p of properties) {
                if (p.ignore || relationNames.has(p.name))
                    continue;
            ...
            for (const rel of relations) {
                const type = rel.type === 'hasMany' || rel.type === 'belongsToMany'
