...
cli.command("clean")
    ...
    .option("--config <path>", "Path to prisma-laravel config file")
    .option("--types <list>", "Comma-separated: migration,model,enum", (v) => v.split(",").map(s => s.trim().toLowerCase()))
    .option("--names <list>", "Comma-separated base names (tables/models/enums)", (v) => v.split(",").map(s => s.trim()))
    ...
    const want = new Set((opts.types?.length ? opts.types : ["migration", "model", "enum"]));
    ...
    if (!dry)
        await pruneEmptyDirs(out.backups);
