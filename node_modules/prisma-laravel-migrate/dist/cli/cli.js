...
function resolveStub(cfg, type, table) {
  ...
  const root = path13.resolve(process.cwd(), cfg.stubDir, type);
  const direct = path13.join(root, `${table}.stub`);
  if (existsSync(direct)) return direct;
  const groups = Array.isArray(cfg.groups) ? cfg.groups : [];
  for (const g of groups) {
    const stubPath = path13.join(root, g.stubFile);
  ...
  const fallback = path13.join(root, "index.stub");
...
var StubMigrationPrinter = class _StubMigrationPrinter {
  ...
  ensureStub(tableName) {
    ...
    const stubPath = resolved ? resolved : this.globalStubPath ? path13.resolve(process.cwd(), this.globalStubPath) : (() => {
...
var BACKUP_ROOT = path13.resolve(process.cwd(), ".prisma-laravel", "backups");
function backupPathFor(targetFile) {
  const rel = path13.relative(process.cwd(), targetFile);
  const full = path13.join(BACKUP_ROOT, rel + ".bak");
  const dir = path13.dirname(full);
  if (!existsSync(dir)) mkdirSync(dir, { recursive: true });
  return full;
}
var cachedConfig;
async function loadConfig2(filePath) {
...
async function prettify(content, opts = {}) {
  ...
  try {
    const base = await loadConfig2(opts.filepathHint) ?? {};
...
async function writeWithMerge(filePath, theirs, type, overwrite = true, currentPath, removeOld = true) {
  ...
  if (/^(<{7}|={7}|>{7})/m.test(mergedText)) {
    console.warn(
      `\u26A0\uFE0F  Merge conflicts in ${path13.relative(
        process.cwd(),
        filePath
      )} \u2014 resolve <<< >>> markers.`
    );
...
async function generateLaravelSchema(options) {
  ...
  const baseOut = cfg.outputDir ? path13.resolve(process.cwd(), cfg.outputDir) : getOutDir(generator);
  ...
  if (cfg.rules) {
    ...
    if (typeof cfg.rules == "string") {
      const rulesModule = await import(path13.resolve(process.cwd(), cfg.rules));
  ...
  const fallbackStubFile = stub ? path13.resolve(process.cwd(), stub) : getStubPath("migration.stub");
  ...
  active.forEach((mig, idx) => {
    ...
    const existingPath = existingFile ? path13.join(baseOut, existingFile) : void 0;
    let fileName = existingFile ?? `${timestamp}_create_${mig.name}_table.php`;
    let filePath = path13.join(baseOut, fileName);
    while (!existingFile && existsSync(filePath)) {
      ...
      filePath = path13.join(baseOut, fileName);
    }
...
var StubModelPrinter = class {
  ...
  ensureModelStub(tableName) {
    ...
    const stubPath = resolved ? resolved : this.globalModelStub ? path13.resolve(process.cwd(), this.globalModelStub) : (() => {
      throw new Error(`No stub found for model '${tableName}'`);
    })();
    if (stubPath === this.#currentModelStub) return;
    const raw = fs.readFileSync(path13.resolve(stubPath), "utf-8").trim();
  ...
  ensureEnumStub(name) {
    ...
    const stubPath = resolved ? resolved : this.globalEnumStub ? path13.resolve(process.cwd(), this.globalEnumStub) : (() => {
...
async function generateLaravelModels(options) {
  ...
  const enumsDir = cfg.outputEnumDir ? path13.resolve(process.cwd(), cfg.outputEnumDir) : path13.resolve(process.cwd(), "app/Enums");
  if (!existsSync(enumsDir)) {
    mkdirSync(enumsDir, { recursive: true });
  }
  const mStub = shared.output?.models ?? cfg.modelStubPath;
  const modelStub = mStub ? path13.resolve(process.cwd(), mStub) : getStubPath("model.stub");
  const eStub = shared.output?.enums ?? cfg.enumStubPath;
  const enumStub = eStub ? path13.resolve(process.cwd(), eStub) : getStubPath("enum.stub");
  ...
  for (const enumDef of enums) {
    ...
    const enumFile = path13.join(enumsDir, `${enumDef.name}.php`);
  ...
  for (const model of models) {
    ...
    const modelFile = path13.join(modelsDir, `${model.className}.php`);
    !cfg.noEmit && writeWithMerge(
      modelFile,
      modelPhp,
      "model",
...
`;
  ...
  hasModelSpecificStub(model) {
    ...
    const base = path13.basename(stubPath);
  ...
  renderModelWithStub(model, ctx, content, body) {
    ...
    const key = model.tableName || model.name;
    const stubPath = resolveStub(this.stubConfig, "ts", key);
    if (!stubPath || path13.basename(stubPath) === "index.stub") {
      const parts = [];
      if (ctx.imports.trim()) parts.push(ctx.imports.trimEnd());
      if (body.trim()) parts.push(body.trimEnd());
      return parts.join("\n\n") + "\n";
    ...
    if (!tmpl) {
      const raw = fs.readFileSync(path13.resolve(stubPath), "utf8").trim();
      tmpl = new Function(
        "model",
        "imports",
        "content",
...
async function generateTypesFromPrisma(options) {
  ...
  const cfg = {
    ...
    stubDir: pick("stubDir") ?? shared.stubDir ?? path13.join(schemaDir, "stubs"),
    groups,
    // TS-specific
...
async function getLaravelGeneratorConfigs(datamodel) {
  ...
  const modCfg = findCfg("prisma-laravel-models");
  const tsCfg = findCfg("prisma-laravel-types");
  return { migCfg, modCfg, tsCfg };
}
async function runGenerators(configPath, skipPrismaGenerate = false) {
  let schemaPrismaPath = path13.resolve(process.cwd(), "prisma/schema.prisma");
  if (existsSync(configPath)) {
    const cfgMod = await loadConfig(configPath);
    const cfg = cfgMod.default ?? cfgMod;
    if (cfg?.schemaPath) {
      schemaPrismaPath = path13.resolve(process.cwd(), cfg.schemaPath);
    }
  }
  ...
  if (skipPrismaGenerate) {
    ...
    await new Promise((resolve2, reject) => {
      ...
      prisma.on("exit", (code) => code !== 0 ? reject(new Error(`prisma generate exited ${code}`)) : resolve2());
    });
    await doRun();
  }
}
...
// src/cli/cli.ts
var cli = new Command();
cli.name("prisma-laravel-cli").description("Initialize and customize Prisma\u2192Laravel generators & stubs").version("0.1.0");
function generatorBlock(base, stubDirRel, extras = []) {
  const name = `${base}s`;
...
cli.command("init").description("Inject generators into schema.prisma and scaffold stubs/").option(
  "-s, --schema <path>",
  "Prisma schema file",
  "prisma/schema.prisma"
).action(async (opts) => {
  const schemaPath = path13.resolve(process.cwd(), opts.schema);
  const schemaDir = path13.dirname(schemaPath);
  const userStubs = path13.join(schemaDir, "stubs");
...
module.exports = {
  ...
  // Global output overrides used by generators:
  // output: {
  //   migrations: "database/migrations",
  //   models:     "app/Models",
  //   enums:      "app/Enums",
  //   // types:   "resources/types/prisma", // <- used by prisma-laravel-types
  // },

  // Per-generator switches:
  // migrate: { noEmit: false },
  // modeler: { noEmit: false },
  // types:   { noEmit: false }, // <- prisma-laravel-types
...
cli.command("customize").alias("c").description("Scaffold per-table stub files from index.stub").option("-s, --schema <path>", "Prisma schema file", "prisma/schema.prisma").option(
  "-t, --types <list>",
  "Comma-separated: migration,model,enum,ts",
  (val) => val.split(",").map((s) => s.trim().toLowerCase()),
  []
).option(
  "-n, --names <list>",
  ...
  const __dirname = path13.dirname(fileURLToPath(import.meta.url));
  const schemaDir = path13.dirname(path13.resolve(process.cwd(), opts.schema));
  const stubRoot = path13.join(schemaDir, "stubs");
  const doBoth = want.includes("migration") && want.includes("model");
  function resolveStubIndex(kind) {
    const userPath = path13.join(stubRoot, kind, "index.stub");
    const fallbackPath = path13.resolve(
      __dirname,
      "../../stubs",
      `${kind}.stub`
    );
    ...
    throw new Error(
      `Missing both user and fallback index.stub for kind "${kind}"`
    );
  }
  for (const t of want) {
    if (t === "enum") {
  ...
  console.log("\u{1F389} Customize complete!");
});
cli.command("gen").description(
  "Run Prisma generate (unless --skipGenerate), then run Laravel + TS generators"
).option("--config <path>", "Path to prisma-laravel config file").option(
  "--skipGenerate",
  "Only run the Laravel/TS generators (no Prisma generate)"
).action(async (opts) => {
  const configPath = opts.config ? path13.resolve(process.cwd(), opts.config) : path13.resolve(process.cwd(), "prisma-laravel.config.js");
  if (!existsSync(configPath)) {
...
function extractTable(fname) {
  return /_create_(.+?)_table\.php$/.exec(fname)?.[1] ?? fname;
}
cli.command("list").description(
  "List generated files. Use --migrations/--models/--enums and --sorted for migration DB order (uses backup baselines)."
).option("--config <path>", "Path to prisma-laravel config file").option("--migrations", "List migrations").option("--models", "List models").option("--enums", "List enums").option(
  "--sorted",
  "For migrations: show tables in dependency order (requires backups present)"
).action(
  async (opts) => {
    const configPath = opts.config ? path13.resolve(process.cwd(), opts.config) : path13.resolve(process.cwd(), "prisma-laravel.config.js");
    if (!existsSync(configPath)) {
    ...
    const cfg = cfgMod.default ?? cfgMod;
    const out = {
      migrations: path13.resolve(
        process.cwd(),
        cfg.output?.migrations ?? "database/migrations"
      ),
      models: path13.resolve(
        process.cwd(),
        cfg.output?.models ?? "app/Models"
      ),
      enums: path13.resolve(
        process.cwd(),
        cfg.modeler?.outputEnumDir ?? cfg.output?.enums ?? "app/Enums"
      ),
      backups: path13.resolve(
        process.cwd(),
        ".prisma-laravel/backups"
      )
    };
    const wantAll = !opts.migrations && !opts.models && !opts.enums;
    const ls = (dir, pred = () => true) => existsSync(dir) ? readdirSync(dir).filter(pred) : [];
    if (wantAll || opts.migrations) {
      ...
      if (opts.sorted) {
        const bakDir = path13.join(
          out.backups,
          path13.relative(process.cwd(), out.migrations)
        );
        const bakFiles = existsSync(bakDir) ? readdirSync(bakDir).filter(
          (f) => f.endsWith(".php")
        ) : [];
        if (!bakFiles.length) {
          console.log(
            "   (no backups found \u2014 cannot show sorted list)"
          );
        } else {
          const ordered = [...bakFiles].sort(
            (a, b) => orderKey(a).localeCompare(orderKey(b))
          );
          console.log(
            "\n   \u{1F522} Backup order (by filename):"
          );
          ordered.forEach(
            (f, i) => console.log(
              `   ${i + 1}. ${extractTable(f)}  (${f})`
            )
          );
        }
      }
...
cli.command("clean").description(
  "Delete generated files & backups (driven by .bak), then re-run generate."
).option("--config <path>", "Path to prisma-laravel config file").option(
  "--types <list>",
  "Comma-separated: migration,model,enum",
  (v) => v.split(",").map((s) => s.trim().toLowerCase())
).option(
  "--names <list>",
  "Comma-separated base names (tables/models/enums)",
  (v) => v.split(",").map((s) => s.trim())
).option(
  "--skipGenerate",
  "Do not re-run generation after cleanup"
).option(
  "--dry-run",
  ...
  async (opts) => {
    ...
    const want = new Set(
      opts.types?.length ? opts.types : ["migration", "model", "enum"]
    );
    const names = new Set(
      (opts.names ?? []).map((s) => s.toLowerCase())
    );
    ...
    const listRecursive = async (dir) => {
      ...
      while (stack.length) {
        ...
        for (const entry of readdirSync(d, {
          ...
          const full = path13.join(d, entry.name);
    ...
    const isBak = (f) => f.endsWith(".bak");
    const stripBak = (p) => p.replace(/\.bak$/i, "");
    const matchMig = (basenameGenerated) => {
      if (!names.size) return true;
      const m = /_create_(.+?)_table\.php$/.exec(
        basenameGenerated
      );
      const table = (m?.[1] ?? "").toLowerCase();
      return !!table && names.has(table);
    };
    const matchModelOrEnum = (basenameGenerated) => {
      if (!names.size) return true;
      return names.has(
        basenameGenerated.replace(/\.php$/i, "").toLowerCase()
      );
    };
    async function cleanByType(kind, outDir, matcher) {
      const bakDir = path13.join(
        out.backups,
        path13.relative(process.cwd(), outDir)
      );
      const bakFiles = (await listRecursive(bakDir)).filter(
        isBak
      );
      if (!bakFiles.length) {
        console.log(
          `(no .bak backups found for ${kind} in ${bakDir})`
        );
        return;
      }
      const plan = bakFiles.map((bakPath) => {
        const relWithBak = path13.relative(bakDir, bakPath);
        const relGen = stripBak(relWithBak);
        const genPath = path13.join(outDir, relGen);
        return {
          bakPath,
          genPath,
          baseGen: path13.basename(relGen)
        };
      }).filter((item) => matcher(item.baseGen));
      if (!plan.length) {
        console.log(
          `(nothing to remove for ${kind}${names.size ? " with current filters" : ""})`
        );
        return;
      }
      let removed = 0;
      ...
      console.log(
        `\u{1F9F9} Removed ${removed} ${kind}${removed === 1 ? "" : "s"}${names.size ? " (filtered)" : ""}`
      );
    }
    if (want.has("migration"))
      await cleanByType("migration", out.migrations, matchMig);
    if (want.has("model"))
      await cleanByType("model", out.models, matchModelOrEnum);
    if (want.has("enum"))
      await cleanByType("enum", out.enums, matchModelOrEnum);
