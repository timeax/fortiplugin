 ...
 */
export async function writeWithMerge(filePath, theirs, type, overwrite = true, currentPath, removeOld = true) {
    const readPath = currentPath ?? filePath; // source for mine/base
    if (!overwrite && existsSync(filePath))
        return;
    const doFormat = (code) => global?._config?.[type]?.prettier
    ...
    const cleanupOld = () => {
        if (moved && existsSync(bakOld))
            safeUnlink(bakOld);
        if (moved && removeOld && existsSync(readPath))
            safeUnlink(readPath);
    };
    ...
    if (theirs === base) {
        ...
        if (moved || destMissing)
            writeFileSync(filePath, (mine ?? theirs), "utf-8");
        writeFileSync(bakNew, theirs, "utf-8");
        cleanupOld();
        return;
    }
    ...
    const mergedLines = diff3.merge((mine ?? "").split(/\r?\n/), (base ?? "").split(/\r?\n/), theirs.split(/\r?\n/), { stringSeparator: "\n" }).result;
    const mergedText = mergedLines.join("\n");
    if (/^(<{7}|={7}|>{7})/m.test(mergedText)) {
        console.warn(`⚠️  Merge conflicts in ${path.relative(process.cwd(), filePath)} — resolve <<< >>> markers.`);
    }
