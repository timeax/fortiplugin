...
class FortiPluginServiceProvider extends ServiceProvider
{
    ...
    private function registerInstallationModules(): void
    {
        ...
        $this->app->singleton(ZipRepository::class, function ($app) {
            $driver = (string)(config('fortiplugin.installations.repositories.zip') ?? 'eloquent');
        ...
        $this->app->singleton(PluginRepository::class, function ($app) {
            $driver = (string)(config('fortiplugin.installations.repositories.plugin') ?? 'eloquent');
            return $driver === 'eloquent'
                ? $app->make(EloquentPluginRepository::class)
                : new InMemoryPluginRepository();
        });

        // ── Token manager (bind concrete + contract alias) ─────────────────────
        $this->app->singleton(InstallerTokenManager::class, fn($app) => new InstallerTokenManager($app->make(HostKeyService::class)));

        $this->app->singleton(TokenContract::class, fn($app) => $app->make(InstallerTokenManager::class));
        ...
        $this->app->singleton(FsContract::class, function ($app) {
            $driver = (string)(config('fortiplugin.installations.files') ?? 'filesystem');
        ...
        $this->app->singleton(RouteWriter::class, fn($app) => new RouteWriter($app->make(LaravelFs::class))
        );

        // ── Background scan dispatcher (safe override) ─────────────────────────
        // If the host provides an implementation class via config, bind it.
        $impl = config('fortiplugin.installations.background_scan_dispatcher');
        ...
        $this->app->scoped(VerificationSection::class, fn($app) => new VerificationSection(
        ...
        $this->app->scoped(ProviderValidationSection::class, fn($app) => new ProviderValidationSection(
            $app->make(InstallationLogStore::class),
            $app->make(AtomicFilesystem::class),
            $app->make(Psr4Checker::class),
        ));

        $this->app->scoped(ComposerPlanSection::class, fn($app) => new ComposerPlanSection(
            $app->make(InstallationLogStore::class),
            $app->make(ComposerInspector::class),
        ));

        $this->app->scoped(VendorPolicySection::class, fn($app) => new VendorPolicySection(
            $app->make(InstallerPolicy::class),
            $app->make(AtomicFilesystem::class),
            $app->make(ComposerInspector::class),
            $app->make(InstallationLogStore::class),
        ...
        $this->app->scoped(DbPersistSection::class, fn($app) => new DbPersistSection(
            $app->make(InstallationLogStore::class),
            $app->make(PluginRepository::class),
        ));

        $this->app->scoped(InstallFilesSection::class, fn($app) => new InstallFilesSection(
            $app->make(InstallerPolicy::class),
            $app->make(InstallationLogStore::class),
            $app->make(AtomicFilesystem::class),
        ));

        $this->app->scoped(UiConfigValidationSection::class, fn($app) => new UiConfigValidationSection(
            $app->make(InstallationLogStore::class),
            $app->make(AtomicFilesystem::class),
        ));
        ...
        $this->app->scoped(RouteWriteSection::class, fn($app) => new RouteWriteSection(
            $app->make(InstallationLogStore::class),
            $app->make(AtomicFilesystem::class),
